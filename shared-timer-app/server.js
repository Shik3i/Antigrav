...
// Force correct Content-Type for assets to prevent "Unable to preload CSS" errors in Docker/Alpine
app.use('/assets', (req, res, next) => {
    if (req.path.endsWith('.css')) res.setHeader('Content-Type', 'text/css');
    if (req.path.endsWith('.js') || req.path.endsWith('.mjs')) res.setHeader('Content-Type', 'application/javascript');
    next();
});
