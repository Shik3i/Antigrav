...
This document outlines critical rules and best practices for AI agents collaborating on the **Shik3i/Antigrav** codebase. Adherence to these guidelines is mandatory to ensure environmental stability and deployment integrity.
...
- **Mandatory Workflow**:
    1. All schema changes must be implemented in `database.js` within the `initializeDatabaseSchema()` function.
...
> [!WARNING]
> **NEVER** execute `git commit` or `git push` without an explicit user request.
...
- **Atomic Transactions**: When handling financial data (Koala Coins) or synchronized game states (Wordle), always use database transactions (`BEGIN`, `COMMIT`, `ROLLBACK`) to ensure data integrity.
...
- **Log Monitoring**: After running `restart_server.bat` or `npm start`, always check the command status/output to ensure the server is actually "Listening on port XXXX" and not crashing.

---
*Last Updated: 2026-04-19 (After the Great Build Error of '26)*
*Signed, Shik3i*
