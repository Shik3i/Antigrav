...
const GlobalBets = () => {
    ...
    const MatchGroup = ({ group, isUpcoming }) => {
        ...
        const t1Logo = findTeamLogo(t1Name);
        const t2Logo = findTeamLogo(t2Name);
        ...
        <div style={{ maxWidth: '900px', margin: '0 auto', padding: '16px 20px', paddingBottom: '80px' }}>
            ...
            <div>
                ...
                {groupedPast.length === 0 ? (
                    ...
                    groupedPast.map(group => <MatchGroup key={group.name + group.date} group={group} isUpcoming={false} />)
                )}
            </div>
        </div>
    );
