...
const Friends = ({ socket, embedded }) => {
    ...
    const fetchFriends = async () => {
        try {
            console.log('[Friends] Fetching /api/friends...');
            const res = await fetch('/api/friends', {
                headers: { 'Authorization': `Bearer ${globalToken}` }
            });
            console.log('[Friends] Fetch complete. Res ok?', res.ok);
            if (res.ok) {
                const data = await res.json();
                console.log('[Friends] Parsed friends data:', data);
                setFriends(data);

                // Get initial online statuses via socket
                // if (socket?.connected) {
                //     console.log('[Friends] Emitting GET_FRIENDS_STATUS from fetch...');
    ...
    useEffect(() => {
        console.log('[Friends] useEffect mounted. isGuest:', isGuest, 'token:', !!globalToken, 'socket connected?', socket?.connected);
        ...
        if (socket) {
            const handleFriendsStatus = (data) => {
                console.log('[Friends] Received FRIENDS_STATUS:', data);
            ...
            console.log('[Friends] Setting up FRIENDS_STATUS listener...', EVENTS, EVENTS.GET_FRIENDS_STATUS, EVENTS.FRIENDS_STATUS);
            socket.on(EVENTS.FRIENDS_STATUS, handleFriendsStatus);
            if (socket.connected) {
                console.log('[Friends] Emitting GET_FRIENDS_STATUS from useEffect...', EVENTS.GET_FRIENDS_STATUS);
                socket.emit(EVENTS.GET_FRIENDS_STATUS);
            }

            return () => {
                console.log('[Friends] UNMOUNTING! Tearing down listener...');
                            ...
                            <>
                                ...
                                <div>
                                    ...
                                    {acceptedFriends.length === 0 ? (
                                        <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>You haven't added any friends yet.</p>
                                    ) : (
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
