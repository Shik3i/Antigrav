    ...
    io.on('connection', (socket) => {
        ...
        socket.on(EVENTS.GET_API_NEWS, async () => {
            try {
                ...
                socket.emit(EVENTS.API_NEWS_DATA, data);
            } catch (err) {
        ...
        socket.on(EVENTS.GET_API_ESPORTS, async () => {
            try {
                ...
                socket.emit(EVENTS.API_ESPORTS_DATA, data);
            } catch (err) {
        ...
        socket.on(EVENTS.GET_API_ALL_TEAMS, async () => {
            try {
                const data = await apiController.fetchAllEsportsTeams();
                socket.emit(EVENTS.API_ALL_TEAMS_DATA, data);
            } catch (err) {
        ...
        socket.on(EVENTS.GET_DB_ESPORTS_TEAMS, async () => {
            try {
                ...
                socket.emit(EVENTS.DB_ESPORTS_TEAMS_DATA, { teams, lastUpdated });
            } catch (err) {
        ...
        socket.on(EVENTS.GET_API_ODDS, async (payload = {}) => {
            try {
                const data = await apiController.fetchPolymarketOddsData(payload?.forceRefreshMatch);
                // If it was a force refresh for a specific match, update everyone! Otherwise, just the requester.
                if (payload?.forceRefreshMatch) {
                    io.emit(EVENTS.API_ODDS_DATA, data);
                } else {
                    socket.emit(EVENTS.API_ODDS_DATA, data);
                }
            } catch (err) {
        ...
        socket.on(EVENTS.JOIN_ROOM, async ({ roomId, userId, displayName, preferences, token }) => {
            try {
                ...
                const safeEmit = (sock, event, data) => {
                    if (!event) {
                ...
                // Initial sync
                safeEmit(socket, EVENTS.SYNC_STATE, roomManager.getRoomState(roomId));
        ...
        socket.on('REQUEST_ROOM_EVENT_SYNC', ({ roomId }) => {
            ...
            if (room) {
                try {
                    JSON.stringify(room.state.eventHistory);
                    socket.emit(EVENTS.ROOM_EVENT_SYNC, room.state.eventHistory);
                } catch (e) {
        ...
        socket.on(EVENTS.DRAW_LINE, ({ roomId, line }) => {
            if (roomManager.drawCanvasLine(roomId, line)) {
                // For performance, we could just emit the line and let clients append it, 
                // but syncing full state is safer for late joiners for this MVP
                io.to(roomId).emit(EVENTS.SYNC_STATE, roomManager.getRoomState(roomId));
            }
        });
        ...
        socket.on(EVENTS.REPORT_METRICS, ({ ping, offset }) => {
            ...
            if (roomId) {
                // We broadcast this periodically or immediately (immediately is fine for MVP)
                io.to(roomId).emit(EVENTS.SYNC_STATE, roomManager.getRoomState(roomId));
            }
        });
